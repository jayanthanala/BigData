...
df3.printSchema()
df4 = df3.dropDuplicates() ## no duplicates 

## Cleaning and Metrics of the dataset
labels=["a real animal","an animal body part"]
df5 = df4.filter(df4.label_product.isin(labels))
df5.limit(5)

df5.select('url').distinct().count()
df5.select('domain').distinct().count()

df5.groupBy(df5.seller)
df5.limit(5)

df5.select('location').distinct().count()
df5.groupBy("location").agg({"location":"count"}).show()

df5.groupBy("country").agg({"country":"count"}).show()

df5.summary("count").show() ## shows count of all the records wrt columns

df5.select([count(when(col(c).contains('None') | \
                            col(c).contains('null') | \
                            (col(c) == '' ) | \
                            col(c).isNull() | \
                            isnan(c), c 
                           )).alias(c)
                    for c in df5.columns]).show() ## shows count of non-null value records
...
parFile1.unionByName(parFile2, allowMissingColumns=True).drop(*cols).dropna().show(200)
...
parFile1 = spark.read.parquet('01_taxa_use_combos.csv.parquet')
...
parFile2 = spark.read.parquet('datasets/02_gbif_taxonomic_key.parquet')
...
parFile3 = spark.read.parquet('datasets/03_gbif_common_names.parquet')
...
df6 = df5.withColumn("animal_names", extract_animal_udf(col("title")))
df6.write.parquet("datasets/df6_animals.parquet") 
## ........................................................
df6.limit(5)
